...
class random_image_opaque(object):
    def __init__(self, opa = [0.2, 1.0], path = '/dev/shm/train_0/', which = [0,10000]):
        self.opa = opa
        self.path = path
        self.which = which
        self.imgs = os.listdir(self.path)
...
transform_q = transforms.Compose(
    [
        ...
        transforms.RandomApply([random_image_opaque()],p=0.4),
...
os.makedirs('/dev/shm/isc_100k_256_big_opa/isc_100k_256_big_opa',exist_ok=True)
#num = 19
...
for i in range(begin,end):
    if(i%10==0):
        ...
        image.resize((256,256)).save('/dev/shm/isc_100k_256_big_opa/isc_100k_256_big_opa/'+name, quality=100)
        for j in range(1,20):
            image_q = transform_q(image)
            name = str(i//10)+'_'+ str(j) +'.jpg'
            image_q.save('/dev/shm/isc_100k_256_big_opa/isc_100k_256_big_opa/'+name, quality=100)
