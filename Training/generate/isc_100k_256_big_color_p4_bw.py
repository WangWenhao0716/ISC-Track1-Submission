...
transform_q = transforms.Compose(
    [
        ToRGB(),
        transforms.RandomApply([RandomColorize()],p=0.4),
...
os.makedirs('/dev/shm/isc_100k_256_big_color_p4_bw/isc_100k_256_big_color_p4_bw',exist_ok=True)
#num = 19
...
for i in range(begin,end):
    if(i%10==0):
        ...
        transform_p(image).resize((256,256)).\
        save('/dev/shm/isc_100k_256_big_color_p4_bw/isc_100k_256_big_color_p4_bw/'+name, quality=100)
        for j in range(1,20):
            image_q = transform_q(image)
            name = str(i//10)+'_'+ str(j) +'.jpg'
            image_q.save('/dev/shm/isc_100k_256_big_color_p4_bw/isc_100k_256_big_color_p4_bw/'+name, quality=100)
